...
if [ -f "./setup_environment.sh" ]; then
    ...
    echo_warning "setup_environment.sh not found. Proceeding without environment setup."
fi
...
max_iterations=2000
write_interval=200
mesher="block"  # Default mesher: block or snappy
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --mesher)
            mesher="$2"
            if [[ "$mesher" != "block" && "$mesher" != "snappy" ]]; then
                echo_error "Invalid mesher option. Use 'block' or 'snappy'."
                exit 1
            fi
            shift 2
            ;;
        --help)
            ...
            echo "  --mesh SIZE       Mesh refinement factor (smaller = finer), default: 0.5"
            echo "  --mesher TYPE     Meshing method: 'block' (default) or 'snappy'"
...
# Calculate cell counts based on mesh refinement factor
# For mesh_size=1.0, we use 100 cells along length and 15 cells across
# Smaller mesh_size means finer mesh (more cells)
...
# Ensure at least 4 cells across
if [ "$CELLS_CROSS" -lt 4 ]; then
    CELLS_CROSS=4
fi

# Calculate cells in each section
...
echo_info "Mesh settings: refinement factor = $MESH_REFINEMENT_FACTOR"
echo_info "Total cells along length: $TOTAL_CELLS_LENGTH, across: $CELLS_CROSS"
...
elif [ "$mesher" == "snappy" ]; then
    # Option 2: Use snappyHexMesh with a coarse background blockMesh
    echo_info "Generating mesh with snappyHexMesh..."
    
    # First create a coarse background mesh with blockMesh
    echo_info "Creating coarse background mesh with blockMesh..."
    blockMesh -case "$case_dir" > "$case_dir/blockMesh.log" 2>&1
    
    if [ $? -ne 0 ]; then
        echo_error "Background blockMesh failed. Check log for details: $case_dir/blockMesh.log"
        cat "$case_dir/blockMesh.log"
        exit 1
    fi
    
    # Create STL file for the aorta geometry using Gmsh
    echo_info "Creating STL geometry for snappyHexMesh..."
    
    # Create triSurface directory first (before running Gmsh)
    mkdir -p "$case_dir/constant/triSurface"
    
    # Run Gmsh to generate an STL file
    gmsh -2 "$case_dir/aorta.geo" -format stl -o "$case_dir/constant/triSurface/aorta.stl" > "$case_dir/gmsh_stl.log" 2>&1
    
    if [ $? -ne 0 ]; then
        echo_error "Gmsh failed to generate STL. Check log: $case_dir/gmsh_stl.log"
        cat "$case_dir/gmsh_stl.log"
        exit 1
    fi
    
# Create a simpler snappyHexMeshDict
    echo_info "Creating snappyHexMeshDict..."
    snappy_dict_file="$case_dir/system/snappyHexMeshDict"
    
    # Use fixed refinement levels instead of calculated values for reliability
    SURFACE_LEVEL=3
    REGION_LEVEL=2
    
    cat > "$snappy_dict_file" << EOL
/*--------------------------------*- C++ -*----------------------------------*\\
| =========                 |                                                 |
| \\\\      /  F ield         | OpenFOAM: The Open Source CFD Toolbox           |
|  \\\\    /   O peration     | Version:  v2412                                 |
...
geometry
{
    ...
    {
        type    searchableBox;
        min     ($INLET_POS -$AORTA_RADIUS -1);
        max     ($OUTLET_POS $AORTA_RADIUS 2);
    }
}
...
EOL
    ...
    verify_file "$snappy_dict_file"
    
    # Create feature extraction dictionary
    echo_info "Creating surfaceFeatureExtractDict..."
    feature_dict_file="$case_dir/system/surfaceFeatureExtractDict"
    
    cat > "$feature_dict_file" << EOL
/*--------------------------------*- C++ -*----------------------------------*\\
| =========                 |                                                 |
| \\\\      /  F ield         | OpenFOAM: The Open Source CFD Toolbox           |
|  \\\\    /   O peration     | Version:  v2412                                 |
...
EOL
    ...
    verify_file "$feature_dict_file"
    
    # Extract features
    echo_info "Extracting surface features..."
    surfaceFeatureExtract -case "$case_dir" > "$case_dir/surfaceFeatureExtract.log" 2>&1
    
    if [ $? -ne 0 ]; then
        echo_error "surfaceFeatureExtract failed. Check log: $case_dir/surfaceFeatureExtract.log"
        cat "$case_dir/surfaceFeatureExtract.log"
        exit 1
    fi
    
    # Run snappyHexMesh
    echo_info "Running snappyHexMesh (this may take a while)..."
    snappyHexMesh -case "$case_dir" -overwrite > "$case_dir/snappyHexMesh.log" 2>&1
    
    if [ $? -ne 0 ]; then
        echo_error "snappyHexMesh failed. Check log: $case_dir/snappyHexMesh.log"
        tail -n 50 "$case_dir/snappyHexMesh.log"
        exit 1
    fi
    
    echo_success "snappyHexMesh completed successfully"
else
    echo_error "Unknown mesher: $mesher"
    exit 1
fi

# Check if Gmsh is needed as a fallback (only for blockMesh)
if [ "$mesher" == "block" ] && { [ ! -d "$case_dir/constant/polyMesh" ] || [ ! -f "$case_dir/constant/polyMesh/points" ]; }; then
    echo_warning "blockMesh output not found, trying with Gmsh as fallback..."
